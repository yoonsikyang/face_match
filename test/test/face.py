        ...
        for idx, (frame, frame_rgb) in enumerate(source):
            ...
            if multi_face_landmarks:
                ...
                for i, point in enumerate(model_points):
                    current_pos = np.matrix(rotation_vector).T * np.matrix(translation_vector)
                    print(current_pos)
                    rotate_points = (point * current_pos) + 10
                    x = int(rotate_points[0])*10
                    y = int(rotate_points[1])*10
                    frame_t1 = cv2.circle(frame_t1, (x, y), 3, (255,0,0),2)
                    frame_t2 = cv2.circle(frame_t2, ((int((point[0]+10)*10) ,int((point[1]+10)*10) )), 3, (255,0,0),2)
